        ...
        for distribution in livingGhostPositionDistributions:
            ...
            for pos in distribution:
                if distribution[pos] > max:
                    ...
                    best_pos = pos
            possible_pos.append(best_pos)
        min = 9999
        move = None
        for action in legal:
            successor_pos = Actions.getSuccessor(pacmanPosition, action)
            for ghost_pos in possible_pos:
                if self.distancer.getDistance(ghost_pos, successor_pos) < min:
                    min = self.distancer.getDistance(ghost_pos, successor_pos)
                    move = action
        return move
